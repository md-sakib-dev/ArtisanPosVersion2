...
import { X, Ticket, Plus, Trash2, AlertTriangle, Hash } from "lucide-react";

// ======================================================
// TYPES
// ======================================================

interface VoucherPaymentEntry {
  id: number;
...
interface VoucherPaymentModalProps {
  open: boolean;
  onClose: () => void;
  onVoucherTotalChange: (total: number) => void;
}
      ...
      >
        ...
        <div className="px-6 py-5">
          <p className="text-sm leading-relaxed text-[#374151]">
            ...
            voucher {entryCount === 1 ? "entry" : "entries"}?
          </p>
          <p className="mt-2 text-sm text-[#6B7280]">
            All added voucher payment data will be removed.
          </p>
        </div>
...
export default function VoucherPaymentModal({
  open,
  onClose,
  onVoucherTotalChange,
}: VoucherPaymentModalProps) {
  // ====================================================
  // STATE
  // ====================================================

  const [entries, setEntries] = useState<VoucherPaymentEntry[]>([]);
  const [giftVoucherAmount, setGiftVoucherAmount] = useState<number>(0);
  const [voucherType, setVoucherType] = useState<string>(VOUCHER_TYPES[0]);
  const [giftVoucherNo, setGiftVoucherNo] = useState<string>("");
  ...
  const handleAddPayment = async () => {
    ...
    if (!giftVoucherAmount || giftVoucherAmount <= 0) {
      setValidationError("Please enter a valid voucher amount");
      return;
    }
    ...
    if (!giftVoucherNo || giftVoucherNo.trim() === "") {
      setValidationError("Please enter a gift voucher number");
      return;
    }

    const newEntry: VoucherPaymentEntry = {
      id: nextId,
      giftVoucherAmount,
      voucherType,
      giftVoucherNo: giftVoucherNo.trim(),
    };
  ...
  const handleDeleteEntry = (id: number) => {
    ...
    // Recalculate total
    const total = updatedEntries.reduce((sum, e) => sum + e.giftVoucherAmount, 0);
    onVoucherTotalChange(total);
  };
  ...
  const handleSave = () => {
    const total = entries.reduce((sum, e) => sum + e.giftVoucherAmount, 0);
    onVoucherTotalChange(total);
    onClose();
  };
  ...
  const handleAmountChange = (value: string) => {
    ...
    setGiftVoucherAmount(isNaN(numericValue) ? 0 : numericValue);
  };
        ...
        >
          ...
          <div className="flex items-center justify-between border-b border-[#E6EAE3] bg-gradient-to-r from-[#10673E] to-[#0D5A35] px-6 py-4">
            <div className="flex items-center gap-3">
              ...
              <div>
                <h2 className="text-lg font-bold text-white">Voucher Payment</h2>
                <p className="mt-0.5 text-xs text-white/70">
                  Add gift voucher payment entries
                </p>
              </div>
            </div>
            <button
            ...
            <div className="grid grid-cols-12 gap-4">
              ...
              <div className="col-span-4">
                ...
                <select
                  ...
                  className="h-10 w-full rounded-lg border border-[#DDE5DF] bg-white px-3 text-sm text-[#17231D] outline-none transition-all focus:border-[#0E9351] focus:ring-2 focus:ring-[#0E9351]/15"
                >
              ...
              <div className="col-span-4">
                ...
                <input
                  type="text"
                  value={giftVoucherNo}
                  onChange={(e) => setGiftVoucherNo(e.target.value)}
            ...
            <div className="max-h-48 overflow-y-auto px-6 pb-4">
              {entries.length === 0 ? (
                <div className="flex flex-col items-center justify-center py-10 text-[#9AA29C]">
                  ...
                  <p className="mt-3 text-sm font-medium">No voucher payment entries yet</p>
                    ...
                    {entries.map((entry) => (
                      ...
                      >
                        <td className="py-3 font-semibold text-[#10673E]">
                          ৳{entry.giftVoucherAmount.toLocaleString("en-IN")}
                        </td>
                        <td className="py-3">
                    ...
                    <tfoot>
                      <tr className="bg-[#F1F8F3] font-semibold">
                        ...
                        >
                          ৳
                          {entries
                            .reduce((sum, e) => sum + e.giftVoucherAmount, 0)
